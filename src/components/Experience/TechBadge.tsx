...
const TechBadge: React.FC<ITechBadge> = ({ tech, delay }) => {
  ...
  return (
    <motion.span
      ...
      transition={{ duration: 0.3, delay }}
      whileHover={{ scale: 1.05, y: -2 }}
      className="group relative inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg shadow-sm hover:shadow-md transition-all duration-200"
      style={{
    ...
    >
      ...
      <div
        className={`w-2 h-2 rounded-full bg-gradient-to-r ${gradient} group-hover:animate-pulse`}
      />
      <span className="text-xs font-medium" style={{ color: "#F5DEB3" }}>{tech}</span>

      {/* Tooltip */}
      <div 
        className="absolute -top-8 left-1/2 transform -translate-x-1/2 px-2 py-1 text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none"
        style={{
